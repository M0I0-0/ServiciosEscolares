...
exports.crearTurno = (req, res) => {
  ...
  db.get("SELECT id FROM turnos ORDER BY id DESC LIMIT 1", [], (err, last) => {
    ...
    db.run(
      `INSERT INTO turnos (folio, tramite, correo, estado, creado_en)
       VALUES (?, ?, ?, 'EN_ESPERA', ?)`,
      [folio, tramite, correo, creado_en],
      (insErr) => {
        if (insErr) {
...
exports.estadoTurno = (req, res) => {
  ...
  db.get(
    ...
    (err, miTurno) => {
      ...
      // 2) Asegurar que existe turno actual del trámite
      asegurarTurnoActual(miTurno.tramite, (e2, actual) => {
        if (e2) return res.status(500).json({ ok: false });

        // 3) Volver a leer mi turno (por si ya pasó a EN_ATENCION)
        db.get(
          "SELECT id, folio, tramite, estado, atendido_en FROM turnos WHERE folio = ?",
          [folio],
          (eReload, miTurno2) => {
            if (eReload) return res.status(500).json({ ok: false });
            miTurno = miTurno2 || miTurno;

            // 4) Próximos ADELANTE = turnos antes que yo (id menor) y EN_ESPERA
            db.all(
              `SELECT folio
               FROM turnos
               WHERE tramite = ?
               ...
               ORDER BY id DESC
               LIMIT 2`,
              [miTurno.tramite, miTurno.id],
              (e3, prevs) => {
                if (e3) return res.status(500).json({ ok: false });

                // 5) Estimado = cantidad delante * 15 min
                db.get(
                  `SELECT COUNT(*) as c
                   FROM turnos
                   WHERE tramite = ?
                  ...
                  (e4, countRow) => {
                    ...
                    // 6) Si mi turno está en atención, calcular restante real de 3 min
    ...
    (err, row) => {
      ...
      db.run(
        ...
        (upErr) => {
          ...
          // si cancelaron el turno actual (en atención), pasar al siguiente
